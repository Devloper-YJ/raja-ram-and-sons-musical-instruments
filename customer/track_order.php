    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        brand: { 'dark': '#0A192F', 'accent': '#B7915F', 'accent-light': '#D4AF37', 'cream': '#FDFBF6' }
                    },
...
<body class="bg-brand-cream font-sans antialiased text-slate-800 relative min-h-screen">
    ...
    <div class="max-w-3xl mx-auto relative z-10 pt-10 pb-20 px-6">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-extrabold font-display text-white tracking-tight">Track Your Order</h1>
                <div class="flex items-center gap-3 mt-1.5">
                    <p class="text-sm text-slate-300 font-medium">Order ID: <span class="text-brand-accent-light font-bold">#ORD-<?php echo str_pad($order['oid'], 4, '0', STR_PAD_LEFT); ?></span></p>
        ...
        <div class="bg-white p-8 md:p-10 rounded-2xl shadow-[0_20px_50px_rgba(10,25,47,0.12)] border border-slate-100">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-5 border-b border-slate-100 pb-6 mb-8 text-center sm:text-left">
                <div class="w-20 h-20 bg-slate-50 rounded-xl overflow-hidden border border-slate-200 flex-shrink-0 shadow-sm p-1">
                    <?php if(!empty($order['image'])): ?>
                        <img src="/uploads/<?php echo htmlspecialchars($order['image']); ?>" class="w-full h-full object-cover rounded-lg">
                    <?php else: ?>
            ...
            <?php else: ?>
                <div class="relative border-l-2 border-brand-accent/30 ml-6 pl-8 space-y-10 pb-4">
